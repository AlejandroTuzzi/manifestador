...
export const DEFAULT_PRICING = {
  ...
  image: {
    // Provisional per-image estimates; actual 2.5 usage is recorded when returned.
    'gpt-image-2.5-sunburst': { '1K': 0.055, '2K': 0.42, '4K': 0.85 },
    'gpt-image-2.5-flare': { '1K': 0.055, '2K': 0.42, '4K': 0.85 },
    'qwen-image-3.0-pro': { '1K': 0.04, '2K': 0.075, inputPerImage: 0.003 },
    ...
    'firered-image-edit-1-1': { '1K': 0.08 },
    // 1K va con calidad media; 2K/4K con alta (el precio escala con los píxeles) — estimados
    'gpt-image-2':        { '1K': 0.055, '2K': 0.42, '4K': 0.85 }
  },
  video: {
    // Alibaba Model Studio Singapore, USD/s. Editable estimates.
    'wan-3': { '480P': 0.05, '720P': 0.10, '1080P': 0.20 },
    'wan-3-prime': { '480P': 0.068, '720P': 0.14, '1080P': 0.28 },
    // modelId → USD por segundo de video según resolución.
    // Equivalentes oficiales aproximados de BytePlus ModelArk para salida 16:9.
    'seedance-2-5':   { '480p': 0.103, '720p': 0.231 },
    // Estimados de BytePlus ModelArk: verificá el precio real en la consola.
    'seedance-2':      { '480p': 0.031, '720p': 0.07, '1080p': 0.157 },
    'seedance-2-mini': { '480p': 0.01, '720p': 0.023 },
    // Tarifas oficiales MiniMax H3, facturadas por segundo de salida.
    'minimax-h3':      { '768P': 0.08, '2K': 0.13 },
    // Google publica ~USD 0,10/s a 720p; 360p cuesta aproximadamente un tercio.
    // 1080p y 4K son reescalados del resultado y conservan esta estimación.
    'gemini-omni-1-1-flash': { '360p': 0.0333, '720p': 0.10, '1080p': 0.10, '4K': 0.10 }
  },
  audio: {
    // USD por cada 1000 caracteres
    'eleven-v3': { per1kChars: 0.15 },
    'eleven-multilingual-v2': { per1kChars: 0.15 }
  },
  music: {
    // USD por pista generada con Suno (estimado; ajustá según tu proveedor)
    perTrack: 0.05
  },
  translate: {
...
export function mergePricing(saved) {
  ...
  out.audio = {};
  for (const [modelId, defaults] of Object.entries(base.audio)) {
    out.audio[modelId] = { ...defaults, ...(saved.audio?.[modelId] || {}) };
  }
  out.music = { ...base.music, ...(saved.music || {}) };
...
export function audioPrice(pricing, chars, modelId = 'eleven-v3') {
  const per1k = pricing.audio?.[modelId]?.per1kChars
    ?? pricing.audio?.['eleven-v3']?.per1kChars
    ?? 0;
  return (chars / 1000) * per1k;
}

export function musicPrice(pricing) {
  return pricing.music?.perTrack ?? 0;
}
