...
export async function generateOpenAIImage({ apiKey, apiModel, prompt, refPaths, aspectRatio, resolution }) {
  ...
  if (refPaths.length) {
    ...
    for (const [index, ref] of refPaths.entries()) {
      const { base64, mime } = await readAsBase64(ref);
      form.append('image[]', new Blob([Buffer.from(base64, 'base64')], { type: mime }), `ref-${index + 1}${path.extname(ref) || '.png'}`);
    }
