    ...
    switch (service) {
      ...
      case 'ark': {
        ...
        return available.length
          ? { ok: true, detail: `Autenticación OK · disponible: ${available.join(' y ')}.` }
          : { ok: true, detail: 'Autenticación OK. Seedream 5 Pro y Seedance 2.5 no aparecen en este proyecto o región.' };
      }
      ...
      case 'wavespeed': {
        ...
        if (isAuthFail(r.status)) return { ok: false, detail: `Key rechazada (HTTP ${r.status}).` };
        if (!r.ok) return { ok: false, detail: `WaveSpeed respondió HTTP ${r.status}.` };
        const json = await r.json().catch(() => ({}));
        const balance = Number(json?.data?.balance);
        return { ok: true, detail: Number.isFinite(balance) ? `Key válida · saldo USD ${balance.toFixed(2)}.` : 'Key válida.' };
      }
