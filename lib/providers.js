...
export async function generateGemini({ apiKey, apiModel, prompt, refPaths, aspectRatio, resolution, supportsSize }) {
...
export async function generateSeedream({ apiKey, apiModel, endpoint, prompt, refPaths, aspectRatio, resolution }) {
...
// ---------------------------------------------------------------------------
// Alibaba DashScope (Qwen-Image-Edit)
// ---------------------------------------------------------------------------

export async function generateQwen({ apiKey, apiModel, endpoint, prompt, refPaths }) {
  if (!apiKey) throw new Error('Falta la API key de DashScope (Alibaba Model Studio). Cargala en Configuración.');
  if (!refPaths.length) throw new Error('Qwen-Edit necesita al menos una imagen de referencia.');
  const base = (endpoint || 'https://dashscope-intl.aliyuncs.com/api/v1').replace(/\/$/, '');
  const content = [];
  for (const ref of refPaths) content.push({ image: await readAsDataUrl(ref) });
  ...
  const res = await apiFetch(`${base}/services/aigc/multimodal-generation/generation`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  }, `Qwen ${apiModel}`);
  ...
  const images = [];
  for (const item of outContent) {
    if (item.image) images.push(await downloadImage(item.image, 'Qwen'));
  }
  if (!images.length) throw new Error(`Qwen no devolvió imágenes — ${JSON.stringify(json).slice(0, 400)}`);
  return images;
}

// ---------------------------------------------------------------------------
// fal.ai (FLUX.1 Kontext [dev]) — safety checker desactivado
// ---------------------------------------------------------------------------

export async function generateFlux({ apiKey, apiModel, prompt, refPaths, aspectRatio }) {
  if (!apiKey) throw new Error('Falta la API key de fal.ai. Cargala en Configuración.');
  if (!refPaths.length) throw new Error('FLUX Kontext es un modelo de edición: necesita una imagen de referencia.');
  const body = {
    prompt,
    image_url: await readAsDataUrl(refPaths[0]),
    num_images: 1,
  ...
  const res = await apiFetch(`https://fal.run/${apiModel}`, {
    method: 'POST',
  ...
  const images = [];
  for (const item of items) {
    if (item.url) images.push(await downloadImage(item.url, 'FLUX Kontext'));
  }
  if (!images.length) throw new Error(`FLUX no devolvió imágenes — ${JSON.stringify(json).slice(0, 400)}`);
  return images;
}
...
export async function testService({ service, key, endpoint, seedreamModelId }) {
  ...
  try {
    switch (service) {
      ...
      // Ark, DashScope y fal no tienen endpoint gratuito de validación:
      ...
      case 'dashscope': {
        const base = (endpoint || 'https://dashscope-intl.aliyuncs.com/api/v1').replace(/\/$/, '');
        const r = await fetch(`${base}/services/aigc/multimodal-generation/generation`, opts({
          method: 'POST',
          headers: { Authorization: `Bearer ${key}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({ model: 'qwen-image-edit', input: {} })
        }));
        if (isAuthFail(r.status)) return { ok: false, detail: `Key rechazada (HTTP ${r.status}).` };
        const txt = (await r.text().catch(() => '')).slice(0, 300);
        if (/InvalidApiKey|Invalid.*API.?key/i.test(txt)) return { ok: false, detail: 'Key inválida.' };
        return { ok: true, detail: 'Autenticación OK.' };
      }

      case 'fal': {
        const r = await fetch('https://fal.run/fal-ai/flux-kontext/dev', opts({
          method: 'POST',
          headers: { Authorization: `Key ${key}`, 'Content-Type': 'application/json' },
          body: JSON.stringify({})
        }));
        if (isAuthFail(r.status)) return { ok: false, detail: `Key rechazada (HTTP ${r.status}).` };
        if (r.status === 422 || r.status === 400) return { ok: true, detail: 'Autenticación OK.' };
        return r.ok
          ? { ok: true, detail: 'Autenticación OK.' }
          : { ok: false, detail: `Respuesta inesperada (HTTP ${r.status}).` };
      }
...
// ---------------------------------------------------------------------------
// Traducción (Gemini Flash Lite — usa la misma key de Google)
// ---------------------------------------------------------------------------

export async function translateText({ apiKey, text, target }) {
  if (!apiKey) throw new Error('Para traducir hace falta la API key de Gemini (Configuración).');
  const lang = target === 'es' ? 'Spanish' : 'English';
  const body = {
    contents: [{
      parts: [{
        text: `Translate the following text to ${lang}. It is a prompt for an image/audio generation model: preserve its meaning, style and any [bracketed audio tags] exactly as brackets (translate the word inside the brackets too). Reply ONLY with the translated text, nothing else.\n\n---\n${text}`
      }]
    }],
...
5. "qwen-edit" → Alibaba Qwen-Image-Edit on Model Studio / DashScope international (per image)
6. "flux-kontext-dev" → FLUX.1 Kontext [dev] on fal.ai (per image/megapixel)
...
Compare with what you find. Reply ONLY with a JSON object (no markdown fences, no commentary) in exactly this shape — include every key, using the found price or keeping the current value if you found no reliable signal:
{
  "image": {
    ...
    "qwen-edit": {"auto": number},
    "flux-kontext-dev": {"1K": number, "auto": number}
  },
  "audio": {"eleven-v3": {"per1kChars": number}},
