...
const FIRERED_TIMEOUT_MS = 10 * 60 * 1000;
...
export async function generateGemini({ apiKey, apiModel, prompt, preface, refPaths, aspectRatio, resolution, supportsSize }) {
  ...
  if (!images.length) {
    ...
    throw new Error(`Gemini no devolvió imagen${finish ? ` (finishReason: ${finish})` : ''}${block ? ` (blockReason: ${block})` : ''}${text ? ` — ${text.slice(0, 300)}` : ''}`);
  }
  return images;
}
...
export async function generateSeedream({ apiKey, apiModel, endpoint, prompt, preface, refPaths, aspectRatio, resolution, nativeResolutionLevels = false }) {
  ...
  const aspectHint = nativeResolutionLevels && aspectRatio && aspectRatio !== 'auto'
    ? `Create the final image in a ${aspectRatio} aspect ratio.`
    : '';
  const body = {
    model: apiModel,
    prompt: [preface, aspectHint, prompt].filter(Boolean).join('\n\n'),
    size: nativeResolutionLevels ? (resolution || '2K') : seedreamSize(aspectRatio, resolution),
...
// ---------------------------------------------------------------------------
// WaveSpeed (FireRed Image Edit 1.1)
// ---------------------------------------------------------------------------

async function uploadWaveSpeedReference({ apiKey, endpoint, filePath, index }) {
  const base = (endpoint || 'https://api.wavespeed.ai/api/v3').replace(/\/$/, '');
  const { base64, mime } = await readAsBase64(filePath);
  const buffer = Buffer.from(base64, 'base64');
  const extension = mime === 'image/jpeg' ? '.jpg' : mime === 'image/webp' ? '.webp' : '.png';
  const filename = `manifestador-ref-${index + 1}${extension}`;
  const ticketResponse = await apiFetch(`${base}/media/uploads`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ filename, size: buffer.length, content_type: mime })
  }, 'WaveSpeed (preparación de referencia)');
  const ticketJson = await ticketResponse.json();
  const ticket = ticketJson?.data || ticketJson;
  if (!ticket?.upload?.url || !ticket?.download_url) {
    throw new Error(`WaveSpeed no devolvió un ticket de subida válido — ${JSON.stringify(ticketJson).slice(0, 300)}`);
  }
  await apiFetch(ticket.upload.url, {
    method: ticket.upload.method || 'PUT',
    headers: ticket.upload.headers || { 'Content-Type': mime },
    body: buffer
  }, 'WaveSpeed (subida de referencia)');
  return ticket.download_url;
}

function waveSpeedOutputUrl(output) {
  if (typeof output === 'string') return output;
  return output?.url || output?.output_url || output?.download_url || '';
}

// El esquema efectivo de WaveSpeed no acepta "16:9": espera "width*height".
// Usamos dimensiones conocidas, múltiplos de 16 y dentro del máximo 1536.
function fireRedSize(aspectRatio) {
  const sizes = {
    '1:1': '1536*1536',
    '2:3': '1024*1536',
    '3:2': '1536*1024',
...
export async function generateFireRed({ apiKey, apiModel, endpoint, prompt, preface, refPaths, aspectRatio }) {
  if (!apiKey) throw new Error('Falta la API key de WaveSpeed para FireRed. Cargala en Configuración.');
  if (!refPaths?.length) throw new Error('FireRed Image Edit 1.1 necesita al menos una imagen de referencia.');
  const base = (endpoint || 'https://api.wavespeed.ai/api/v3').replace(/\/$/, '');
  const images = await Promise.all(refPaths.slice(0, 3).map((filePath, index) =>
    uploadWaveSpeedReference({ apiKey, endpoint: base, filePath, index })));
  const modelPath = String(apiModel || 'wavespeed-ai/firered-image-v1.1/edit').replace(/^\/+/, '');
  const submitUrl = /^https?:\/\//i.test(modelPath) ? modelPath : `${base}/${modelPath}`;
  const body = {
    prompt: [preface, prompt].filter(Boolean).join('\n\n'),
    images,
    enable_safety_checker: false
  };
  const outputSize = fireRedSize(aspectRatio);
  if (outputSize) body.size = outputSize;
  const submitResponse = await apiFetch(submitUrl, {
    method: 'POST',
    headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(body)
  }, 'FireRed Image Edit 1.1');
  const submittedJson = await submitResponse.json();
  let prediction = submittedJson?.data || submittedJson;
  if (!prediction?.id && prediction?.status !== 'completed') {
    throw new Error(`FireRed no devolvió un ID de tarea — ${JSON.stringify(submittedJson).slice(0, 300)}`);
  }

  const deadline = Date.now() + FIRERED_TIMEOUT_MS;
  while (prediction?.status !== 'completed') {
    if (['failed', 'cancelled', 'timeout', 'deleted'].includes(prediction?.status)) {
      throw new Error(`FireRed terminó en ${prediction.status}${prediction.error ? ` — ${prediction.error}` : ''}`);
    }
    if (Date.now() >= deadline) {
      throw new Error(`FireRed: la tarea ${prediction.id} superó los ${FIRERED_TIMEOUT_MS / 60000} minutos y puede continuar procesándose en WaveSpeed.`);
    }
    await new Promise((resolve) => setTimeout(resolve, 2000));
    const resultResponse = await apiFetch(`${base}/predictions/${encodeURIComponent(prediction.id)}/result`, {
      headers: { Authorization: `Bearer ${apiKey}` }
    }, 'FireRed (consulta de resultado)');
    const resultJson = await resultResponse.json();
    prediction = resultJson?.data || resultJson;
  }

  const urls = (prediction.outputs || []).map(waveSpeedOutputUrl).filter(Boolean);
  if (!urls.length) throw new Error(`FireRed terminó sin imágenes — ${JSON.stringify(prediction).slice(0, 300)}`);
  return Promise.all(urls.map((url) => downloadImage(url, 'FireRed')));
}
...
export async function generateOpenAIImage({ apiKey, apiModel, prompt: userPrompt, preface, refPaths, aspectRatio, resolution, options = {} }) {
  ...
  const { openAIImageOptions, openAIImageUsageCost } = await import('./openai-image-options.js');
  const imageOptions = openAIImageOptions(options, apiModel, resolution);
  ...
  if (refPaths.length) {
    ...
    form.append('background', imageOptions.background);
    form.append('output_format', imageOptions.output_format);
    ...
    res = await apiFetch('https://api.openai.com/v1/images/generations', {
      method: 'POST',
      headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ model: apiModel, prompt, size, ...imageOptions, moderation: 'low', n: 1 })
    }, `OpenAI ${apiModel}`);
  }
  ...
  if (apiModel.startsWith('gpt-image-2.5-')) images.usageCost = openAIImageUsageCost(json.usage);
  return images;
}
...
export async function testService({ service, key, endpoint }) {
  const result = (ok, detailCode, detailParams = {}, detail = '') => ({ ok, detailCode, detailParams, detail });
  if (!key) return result(false, 'config.connection.noKey', {}, 'No hay API key cargada.');
  ...
  try {
    switch (service) {
      case 'wan': {
        ...
        const response = await fetch(`${base}/api/v1/uploads?action=getPolicy&model=wan3.0-video`, opts({ method: 'GET', redirect: 'error', headers: { Authorization: `Bearer ${key}` } }));
        const body = await response.json().catch(() => ({}));
        return response.ok && body.data?.upload_host && body.data?.policy
          ? result(true, 'wan.testAvailable')
          : result(false, 'errors.wanRequest', { detail: String(body.message || body.code || `HTTP ${response.status}`).replaceAll(key, '[redacted]').slice(0, 500) });
      }
      case 'qwen': {
        // Read-only catalog query: never generate an image as a connection test.
        // https://www.alibabacloud.com/help/en/model-studio/list-models
        let base;
        try { base = qwenEndpoint(endpoint); }
        catch { return result(false, 'errors.qwenEndpoint'); }
        const r = await fetch(`${base}/api/v1/models?model=qwen-image-3.0-pro&page_no=1&page_size=20`, opts({
          method: 'GET', redirect: 'error', headers: { Authorization: `Bearer ${key}` }
        }));
        const body = await r.json().catch(() => null);
        const code = String(body?.code || body?.error?.code || '');
        const detail = String(body?.message || body?.error?.message || code || `HTTP ${r.status}`).replaceAll(key, '[redacted]').slice(0, 600);
        if (r.status === 401 || /InvalidApiKey|InvalidAccessToken|Unauthorized/i.test(code)) {
          return result(false, 'config.connection.qwenRejected', { detail });
        }
        if (r.status === 403) return result(false, 'config.connection.qwenForbidden', { detail });
        if (!r.ok || body?.success === false || code || body?.error || !Array.isArray(body?.output?.models)) {
          return result(false, 'config.connection.qwenInconclusive', { detail });
        }
        const found = body.output.models.some((item) => item?.model === 'qwen-image-3.0-pro');
        return found
          ? result(true, 'config.connection.qwenAvailable')
          : result(false, 'config.connection.qwenNotListed');
      }
      case 'gemini': {
        const r = await fetch('https://generativelanguage.googleapis.com/v1beta/models?pageSize=1',
          opts({ headers: { 'x-goog-api-key': key } }));
        return r.ok
          ? result(true, 'config.connection.validKey', {}, 'Key válida.')
          : result(false, 'config.connection.rejected', { status: r.status }, `Key rechazada (HTTP ${r.status}).`);
      }
      ...
      case 'googleTranslate': {
        ...
        return r.ok
          ? result(true, 'config.connection.translationEnabled', {}, 'Google Cloud Translation habilitado.')
          : result(false, 'config.connection.translationRejected', { status: r.status }, `Key rechazada o API no habilitada (HTTP ${r.status}).`);
      }
      ...
      case 'elevenlabs': {
        ...
        if (r.ok) {
          ...
          const detail = j.character_limit
            ...
            : 'Key válida.';
          return j.character_limit
            ? result(true, 'config.connection.elevenUsage', {
              tier: j.tier || '?', used: j.character_count ?? '?', limit: j.character_limit
            }, detail)
            : result(true, 'config.connection.validKey', {}, detail);
        }
        ...
        const r2 = await fetch('https://api.elevenlabs.io/v1/voices',
          opts({ headers: { 'xi-api-key': key } }));
        if (r2.ok) return result(true, 'config.connection.restrictedKey', {}, 'Key válida (permisos restringidos: no puedo leer el plan).');
        return result(false, 'config.connection.rejected', { status: r.status }, `Key rechazada (HTTP ${r.status}).`);
      }
      ...
      case 'ark': {
        ...
        if (isAuthFail(r.status)) return result(false, 'config.connection.rejected', { status: r.status }, `Key rechazada (HTTP ${r.status}).`);
        if (!r.ok) return result(false, 'config.connection.providerHttp', { provider: 'ModelArk', status: r.status }, `ModelArk respondió HTTP ${r.status}.`);
        const catalog = await r.json().catch(() => ({}));
        const models = Array.isArray(catalog.data) ? catalog.data : [];
        const seedance25 = models.some((item) => item.id === 'dreamina-seedance-2-5-260628');
        const seedreamPro = models.some((item) => item.id === 'dola-seedream-5-0-pro-260628');
        const available = [seedreamPro && 'Seedream 5 Pro', seedance25 && 'Seedance 2.5'].filter(Boolean);
        return available.length
          ? result(true, 'config.connection.arkAvailable', { models: available.join(' · ') }, `Autenticación OK · disponible: ${available.join(' y ')}.`)
          : result(true, 'config.connection.arkUnavailable', {}, 'Autenticación OK. Seedream 5 Pro y Seedance 2.5 no aparecen en este proyecto o región.');
      }

      case 'wavespeed': {
        const base = (endpoint || 'https://api.wavespeed.ai/api/v3').replace(/\/$/, '');
        const r = await fetch(`${base}/balance`, opts({ headers: { Authorization: `Bearer ${key}` } }));
        if (isAuthFail(r.status)) return result(false, 'config.connection.rejected', { status: r.status }, `Key rechazada (HTTP ${r.status}).`);
        if (!r.ok) return result(false, 'config.connection.providerHttp', { provider: 'WaveSpeed', status: r.status }, `WaveSpeed respondió HTTP ${r.status}.`);
        const json = await r.json().catch(() => ({}));
        const balance = Number(json?.data?.balance);
        return Number.isFinite(balance)
          ? result(true, 'config.connection.balance', { balance: balance.toFixed(2) }, `Key válida · saldo USD ${balance.toFixed(2)}.`)
          : result(true, 'config.connection.validKey', {}, 'Key válida.');
      }
      ...
      case 'minimax': {
        ...
        if (isAuthFail(r.status)) return result(false, 'config.connection.rejected', { status: r.status }, `Key rechazada (HTTP ${r.status}).`);
        return result(true, 'config.connection.minimaxOk', {}, 'Autenticación MiniMax OK.');
      }
      ...
      case 'suno': {
        ...
        if (r.ok) {
          ...
          return credits != null
            ? result(true, 'config.connection.credits', { credits }, `Key válida — ${credits} créditos.`)
            : result(true, 'config.connection.validKey', {}, 'Key válida.');
        }
        return result(true, 'config.connection.savedNoCredits', {}, 'Key guardada (no pude leer créditos; probá generando).');
      }

      default:
        return result(false, 'config.connection.unknownService', { service }, `Servicio desconocido: ${service}`);
    }
  } catch (err) {
    if (err.name === 'TimeoutError') return result(false, 'config.connection.timeout', {}, 'La petición superó los 20s.');
    if (service === 'qwen' || service === 'wan') {
      const error = String(err.message || '').replaceAll(key, '[redacted]').slice(0, 600);
      return result(false, 'config.connection.networkError', { error });
    }
    return result(false, 'config.connection.networkError', { error: err.message }, `Error de red — ${err.message}`);
  }
}
...
4. "seedream-5-pro" → ByteDance Seedream 5.0 Pro on BytePlus ModelArk (per image at 1K and 2K)
5. "seedream-5-lite" → ByteDance Seedream 5.0 Lite on BytePlus ModelArk (per image)
6. "firered-image-edit-1-1" → FireRed Image Edit 1.1 through WaveSpeed (per generated image)
7. "eleven-v3" → ElevenLabs Eleven v3 TTS, approximate USD per 1000 characters on a paid plan (e.g. Creator)
8. "eleven-multilingual-v2" → ElevenLabs Multilingual v2 TTS, approximate USD per 1000 characters on the same plan
9. "seedance-2-5" → ByteDance Seedance 2.5 directly on BytePlus ModelArk, approximate USD per generated second at 480p and 720p for a common 16:9 output

Also include qwen-image-3.0-pro: direct Alibaba Model Studio Singapore prices per output image (1K, 2K) and per input reference (inputPerImage). No reseller prices.
