...
export async function generateSeedanceVideo({ apiKey, apiModel, endpoint, prompt, refPaths, aspectRatio, resolution, duration, audio }) {
  if (!apiKey) throw new Error('Falta la API key de BytePlus ModelArk (Ark). Cargala en Configuración.');
  const base = (endpoint || 'https://ark.ap-southeast.bytepluses.com/api/v3').replace(/\/$/, '');
  ...
  if (refPaths[0]) content.push({ type: 'image_url', image_url: { url: await readAsDataUrl(refPaths[0]) }, role: 'first_frame' });
  if (refPaths[1]) content.push({ type: 'image_url', image_url: { url: await readAsDataUrl(refPaths[1]) }, role: 'last_frame' });
...
export async function testService({ service, key, endpoint, seedreamModelId }) {
  ...
  try {
    switch (service) {
      ...
      case 'fal': {
        return { ok: true, detail: 'Key guardada. fal.ai queda reservado para futuros modelos.' };
      }
